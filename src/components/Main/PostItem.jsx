...
export default function PostItem({ data }) {
  ...
  async function handleClickLike(e, userId, postId) {
    ...
    if (isLiked) {
      removeLikes([
        likeList?.myLikePost.find((item) => item.postId._id === postId)._id,
      ]);
    } else {
            ...
            <div className='pb-[20px]'>
              <div>
                <div className='relative'>
                  ...
                  <ImageSlide
                    images={item.schedules.flatMap((schedule) =>
                      schedule.flatMap((place) => place.placeImageSrc)
                    )}
              ...
              <div className='flex justify-between items-center'>
                <p className='태안'>{item.destination}</p>
              ...
              <span className='text-gray-1 mb-[4px]'>
                일정: {new Date(item.startDate).getFullYear()}년{' '}
                {new Date(item.startDate).getMonth() + 1}월{' '}
                {new Date(item.startDate).getDate()}일 ~{' '}
                {new Date(item.endDate).getFullYear()}년{' '}
                {new Date(item.endDate).getMonth() + 1}월{' '}
                {new Date(item.endDate).getDate()}일
              </span>
              <p className='text-gray-1'>
                예산: {item.cost.toLocaleString()}원
              </p>
            </div>
          </div>
        </Link>
