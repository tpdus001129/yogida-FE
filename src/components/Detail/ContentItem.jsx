      ...
      <div className='flex flex-col items-center relative'>
        <div
          className={`w-[16px] h-[16px] rounded-full ${
            firstPlace ? 'bg-[#E8E8E8]' : 'bg-secondary'
          }`}
        ></div>
        <div className='w-[3px] h-[144px] bg-[#E8E8E8] mt-[-1px]'></div>
        {lastPlace ? (
          <div className='w-[16px] h-[16px] rounded-full bg-secondary'></div>
        ) : (
          ''
        )}
        <div className='absolute bottom-[40px] bg-[#ffffff] flex items-center'>
          {distance ? (
            <IoCar size='26px' color='#E8E8E8' />
          ) : (
            <IoWalkOutline size='26px' color='#E8E8E8' />
          )}
      ...
      <div className='w-[236px] h-[130px] bg-[#ffffff] rounded-[20px] drop-shadow-[0_4px_4px_rgba(0,0,0,0.25)]'>
        <div className='h-[43px] flex justify-between items-center mx-[16px] rounded-t-[20px]'>
          ...
          <button onClick={handleClick}>
            {isClicked ? (
              <IoBookmark color='#FFDB5F' size='17' />
            ) : (
              <IoBookmarkOutline color='#FFDB5F' size='17' />
            )}
          </button>
        </div>
...
ContentItem.propTypes = {
  firstPlace: PropTypes.bool.isRequired,
